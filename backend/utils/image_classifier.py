...
class Dataset:
    ...
    @property
    def labels(self) -> List[type]: return self.data[self.class_feature]

    @property
    def factorized_labels(self) -> List[type]: return pd.factorize(self.labels)[0]
...
class ImageClassifier:
    ...
    def evaluate_with_random_forest(self, split_ratio: float, n_jobs: int = 2, random_state: int = 0):
        ...
        train, test = self.split_data(split_ratio=split_ratio)  # type, base de données
